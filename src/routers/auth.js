...
const validate = [
  check('name').isLength({ min: 2 }).withMessage('Your full name is required'),
...
const generateToken = (user) => {
  return jwt.sign({ _id: user._id, email: user.email, name: user.name }, secretKey);
};
...
router.post('/signup', validate, async (req, res) => {
  ...
  if (userExist) return res.status(400).send({ succes: false, message: 'Email already exists' });
